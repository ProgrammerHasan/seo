...
final class SeoManager implements Htmlable
{
    ...
    {
        if ($model instanceof Seoable || method_exists($model, 'toSeo')) {
            return $this->set($model->toSeo());
        }
    ...
    {
        $this->data->canonical($url ?: UrlCanonicalizer::current());
    ...
    public function schema(?array $schema = null): self|Schema
    {
        if ($schema === null) {
            return new Schema;
    ...
    public function schemaBuilder(): Schema
    {
        return new Schema;
    }

    public function schemaFactory(): Schema
    {
        return new Schema;
    }

    public function ai(): SeoAi
    {
        return new SeoAi((string) config('seo.ai.default_driver', 'local'));
    }

    public function inertia(SeoData|SeoBuilder|array|null $seo = null): array
    {
        return (new InertiaSeo)->head($seo ?: $this->data);
    }
    ...
    {
        return (new InertiaSeo)($this->data);
    }
}
