...
final class SeoBuilder
{
    ...
    public function canonical(?string $url = null): self { $this->data->canonical($url ?: url()->current()); return $this; }
    ...
    public function image(string $url, ?string $alt = null, ?int $width = null, ?int $height = null): self
    {
        ...
        if ($alt) $this->data->og('image:alt', $alt);
        if ($width) $this->data->og('image:width', $width);
        if ($height) $this->data->og('image:height', $height);
        return $this;
    }
    ...
    public function article(?string $published = null, ?string $modified = null, array $tags = []): self
    {
        ...
        if ($published) $this->data->og('article:published_time', $published);
        if ($modified) $this->data->og('article:modified_time', $modified);
        foreach ($tags as $tag) $this->data->og('article:tag', $tag);
        return $this;
    }
    ...
    public function schema(array $schema): self { $this->data->schema($schema); return $this; }
    ...
    public function data(): SeoData { return $this->data; }
    public function toArray(): array { return $this->data->toArray(); }
    public function forInertia(): array { return $this->toArray(); }
    public function generate(): string { return $this->toHtml(); }
    public function toHtml(): string { return ($this->renderer ?: new HtmlRenderer())->render($this->data); }
}
